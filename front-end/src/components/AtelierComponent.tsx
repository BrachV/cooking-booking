export const AtelierComponent = ({ ateliers, admin }: { ateliers: any, admin?: boolean }) => {
	return (
		<div className="flex w-4/6 bg-white p-4 mb-4 border justify-between">
			<div className="flex">
				<img className="h-auto w-44 ml-1" src={`/logo/${ateliers.theme}.png`} alt="Drapeau" />
				<div className="flex flex-col justify-center ml-6">
					<h2 className="text-xl font-semibold">{ateliers.title}</h2>
					<p className="text-gray-500">{ateliers.description}</p>
					<p className="text-gray-500">{ateliers.nbPlaces} places disponibles</p>
